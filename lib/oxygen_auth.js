...
function Strategy(options, verify) {
  ...
  this.name = 'oxygen';
  this._verify = verify;
  this.validation_url = 'https://accounts.autodesk.com/api/oauth/v1/validateAuthorization';
  ...
  try {
    ...
    if ( !isEmptyObject( req.body ) ){
      ...
      for (var key in req.body) {
        ...
        // remove internal url-encoded backslashes and leading and trailing url encoded quotes
        param_string = param_string + encodeURIComponent( req.body[key] ).replace("%5C","");
      }
      encodedParams = param_string; // encodeURIComponent(param_string);
      if (debug){
