...
var S3_BUCKET_NAME = process.env.NODE_ENV === "development" ? "greg-pkgs-dev" : "greg-pkgs-prod";
 ...
 *
 * @param {ObjectId} The id of the pkg
 * @param {Function} Callback to execute after white listing. The arguments are an error object (null if successful) and the white listed pkg.
 * @api private
 */
exports.whitelist_by_id = function(pkg_id, callback){
	PackageModel.findById(pkg_id, function(err,pkg){
		if(err){
			if(callback){
				callback(error.fail('Failed to find the package in the db.'))
				return;
			}
		}

	    pkg.white_listed = true;
	    pkg.save(function(err){
	      if(err){
	      	if(callback){
	      		callback(error.fail('There was an error white-listing the package.'));
	      		return;
	      	}
	      }
	    });
 ...
 *
 * @param {ObjectId} The id of the pkg
 * @param {Function} Callback to execute after removal from the white list. The arguments are an error object (null if successful) and the pkg removed from the white list.
 * @api private
 */
