...
exports.download_pkg_and_deps = function(req, pkg_id, callback ) {
...
function _validate_new_pkg_version(req, pkg_data, callback) {
	...
	if (!pkg_data.contents) {
		callback( error.fail('You must supply the package contents.') );
		return;
	}
...
function _save_new_pkg_version(pkg_vers_data, callback) {
	...
	async.series([
	   ...
	   PackageModel.findById(pkg.pkg_id, function(err, pkg) {
		...
		if (callback) callback( error.success_with_content( 'Successfully inserted the package in the db', pkg_v) );
	});
