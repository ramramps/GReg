	...
	async.waterfall([ 
		...
		function(user, outer_callback) {
			...
			PackageModel.findOne({name: pkg_data.name, engine: pkg_data.engine}, function(err, pkg) {
				...
				console.log('ASSIGNING PACKAGE ID TO NEW PKG VERSION: ', pkg._id)
		...
		function(user, pkg, outer_callback) {
			...
			for (var i = 0; i < user.maintains.length; i++) {
				console.log('TESTING', user.maintains[i], 'to', pkg._id );
			...
			if ( !user_is_maintainer ){
				callback( error.fail('The user sending the new package version, '+ user.username + ', is not a maintainer of the package ' + pkg.name + pkg._id + user.maintains) );
				return;
			}
			outer_callback(null, user, pkg);
		},
		...
		// TODO: populate all dependencies
	...
	async.waterfall([
		...
		function(inner_callback){
			...
			if (pkg_data.direct_dependency_ids != undefined && pkg_data.direct_dependency_ids.length > 0 )
				exports.update_pkg_list_used_by( pkg, pkg_data.direct_dependency_ids, inner_callback );
			else
