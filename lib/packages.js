...
var S3_BUCKET_NAME = 'greg-server';
  ...
  PackageModel.findById( pkg_id, function(err, pkg) {
    ...
    UserModel.findById( user_id, function(err, user){
      ...
      if (change < 0){
	      ...
	      // indicate the user has voted for this before  
	      ...
	      if (user.has_downvoted && user.has_downvoted.indexOf(pkg_id) != -1 ){
	...
	async.waterfall([ 
	   ...
	   PackageModel.findById(pkg_data.pkg_id, function(err, pkg) {
		...
		function(pkgs, inner_callback){
			...
			callback( error.success_with_content( 'Successfully inserted the package in the db', pkg) );
