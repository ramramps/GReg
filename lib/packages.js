...
exports.comment = function(pkg_id, user_id, comment, res) {
  ...
  PackageModel.findById( pkg_id, function(err, pkg) {
    ...
    pkg.comments.push({user: user_id, text: comment});
      ...
      // at this point, the user is doing one of the following
	      ...
	      // 1) has a recorded upvote and is now downvoting to neutral
			...
			if (user.has_upvoted && user.has_upvoted.indexOf(pkg_id) != -1 ){
				...
				user.has_upvoted.remove( user.has_upvoted.indexOf(pkg_id) );
				...
				user.has_downvoted.remove( user.has_downvoted.indexOf(pkg_id) );
...
exports.get_pkg_search_object = function(pkg) {
	...
	if (pkg.name){
		...
		// build title suffix arrays for substring searches
		_.each(pkg.name.split(" "), function(word) {
			if ( word.length <= 3 ) return;
			for (var i = 1; i < word.length-1; i++){
				pkg_search_obj.name += (" " + word.substring(i) );
			}
		});
	}
	   ...
	   PackageModel.findById(pkg_data.pkg_id, function(err, pkg) {
		...
		function(pkg, inner_callback) {
			search.update(pkg._id, exports.get_pkg_search_object(pkg), function(err) {
				if (err) {
					return inner_callback(error.fail('Failed to update the search database'))
				}
				inner_callback(null, pkg);
			});
		},
