var PackageModel = require('../lib/models/package').PackageModel
	, error = require('./error')
	, users = require('./users')
	, async = require('async')
	, _ = require('underscore')
	, mongoose = require('mongoose')
	, UserModel = require('../lib/models/user').UserModel
    , fs = require('fs')
 ...
 * @api private
 */
exports.whitelist_by_id = function(pkg_id, add, callback){
	PackageModel.findById(pkg_id, function(err,pkg){
		if(err){
			if(callback){
	    ...
	    pkg.white_listed = add;
        
	    pkg.save(function(err){
	      if(err){
	      	if(callback){
	      		callback(error.fail('There was an error setting the white-list value for package.'));
	      		return;
	      	}
	      }
	    });
