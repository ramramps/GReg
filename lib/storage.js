var storage = require('./models').PackageModel
	, error = require('./error')
	, users = require('./users')
	, async = require('async')
...
exports.mockBucket = './test/mock_bucket/';
 ...
 */
exports.upload = process.env.NODE_ENV != "production" ? saveToDisk : uploadToS3;
...
function saveToDisk(req, pkg_data, guid){
    ...
    var objectName = guid + req.files.pkg.name;
     ...
     fs.copy(req.files.pkg.path, exports.mockBucket + objectName, function(err){
        if(err) {
            throw err;
        };
    });
