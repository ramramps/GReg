	...
	UserModel
		...
		.exec(function(err, users){
	        ...
	        user.maintains = user.maintains.filter(function(elem, pos) {
					...
					user.num_votes_for_maintained_packages =  _.reduce(user.maintains, function(memo, pkg){ return memo + pkg.downloads; }, 0);
					user.markModified('num_votes_for_maintained_packages'); 

					user.num_maintained_packages = user.maintains.length;
					user.markModified('num_maintained_packages'); 

					user.most_recent_change = new Date( _.max(user.maintains, function(pkg){ return pkg.latest_version_update.getTime(); }) );
	        user.markModified('most_recent_change');     
...
exports.reconstruct_internal_user_stats(function(err){
	if (err) return console.error(err);
	console.log('success');
});
