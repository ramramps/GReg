...
exports.most_recently_active_authors = function( limit, callback){
	...
	return user_sort_by('username', 100, function(err, users){
		...
		users.sort(function(a_d,b_d){
			...
			var b = new Date(b_d.last_updated_package.latest_version_update);	
...
function user_sort_by(field_to_sort_on, limit, callback){
	...
	UserModel
		...
		.select('username last_updated_package num_maintained_packages num_votes_for_maintained_packages num_downloads_for_maintained_packages')
