	...
	async.parallel( [
		...
		function(inner_callback){
			exports.all_user_stats(limit, function(err, user_stats){
				...
				inner_callback(null, user_stats);
			});
		}
	, function(user_stats, inner_callback){
			exports.all_engine_stats(null, limit, function(err, pkg_stats){
				if (err) return inner_callback(error);
				inner_callback(null, user_stats, pkg_stats);
			});
		}
		}], function(err, user_stats, pkg_stats){ 
			if (err) return callback(err);

			// merge properties from user_stats into pkg_stats
			for (var attrname in user_stats) { pkg_stats[attrname] = user_stats[attrname]; }
			callback(null, pkg_stats);
	});
